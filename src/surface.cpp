...
void CreateSwapchain(State* state, VkSwapchainKHR handle)
{
  ...
  validate(
    vkCreateSwapchainKHR(
      state->context->device, &swapchain_info, NULL, &state->swapchain->handle),
  ...
  //     (VkImage *)ArenaPush(&state->swapchain_arena, sizeof(VkImage) *
  //     state->swapchain->image_count);
  ...
  //     (VkImageView *)ArenaPush(&state->swapchain_arena, sizeof(VkImageView) *
  //     state->swapchain->image_count);
  ...
  for (u32 i = 0; i < state->swapchain->image_count; i++)
  {
    ...
    validate(
      vkCreateImageView(
        state->context->device, &view_info, NULL, &state->swapchain->views[i]),
  ...
  //     (VkSemaphore *)ArenaPush(&state->swapchain_arena, sizeof(VkSemaphore) *
  //     state->swapchain->image_count);
...
void RecreateVulkanSwapchain(State* state)
{
  ...
  vmaDestroyImage(state->context->allocator,
                  old_swapchain.depth_image,
                  old_swapchain.depth_alloc);

  for (u32 i = 0; i < old_swapchain.image_count; i++)
  {
    vkDestroySemaphore(state->context->device,
                       old_swapchain.begin_presenting_semaphore[i],
                       NULL);
